function [U,DU,DDU,D2U] = polyhedronWS(FP,faces,vertices,density,plotflag)

% <strong>polyhedronWS</strong> - Gravity effects of polyhedron model.
%
%   [U,DU,DDU,D2U] = polyhedronWS(FP,faces,vertices,density,plotflag)
%
% -------------------------------------------------------------------------
...
%       density=2e3;
%       % call the function
%       [U,DU,DDU,D2U] = polyhedronWS(FP,FV.faces,FV.vertices,density,1);
% -------------------------------------------------------------------------
%
...
% ###############################################
%
% Function :  polyhedronWS (Version 1.0)
...
%                     - First release.
%
%       2016-09-18 - v1.1 - Andrea Capannolo
%                     - Introduction of potential third derivatives matrix
%
...
U_edge_old=0;
DU_edge_old=zeros(3,1);
DDU_edge_old=zeros(3,3);

for w=1:nedges  % for each edge
    ...
    % sum over the edges
    U_edge  = Uj_edge   + U_edge_old;
    DU_edge = DUj_edge  + DU_edge_old;
    DDU_edge= DDUj_edge + DDU_edge_old;

    % next iteration
    U_edge_old  = U_edge;
    DU_edge_old = DU_edge;
    DDU_edge_old= DDU_edge;
...
U_face_old=0;
DU_face_old=zeros(3,1);
DDU_face_old=zeros(3,3);
D2U_face_old=0;

for h=1:nfaces  % for each face
    ...
    % sum over the faces
    U_face   = Uj_face    + U_face_old;
    DU_face  = DUj_face   + DU_face_old;
    DDU_face = DDUj_face  + DDU_face_old;
    D2U_face  = D2Uj_face + D2U_face_old;
    
    % next iteration
    U_face_old   = U_face;
    DU_face_old  = DU_face;
    DDU_face_old = DDU_face;
    D2U_face_old  = D2U_face;
